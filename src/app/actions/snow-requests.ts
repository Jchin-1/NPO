...
/**
 * Server action to update the status of a snow removal request
 * Used by admin to mark requests as completed, pending, or declined
 * 
 * @param requestId - The ID of the request to update
 * @param status - The new status ('pending', 'confirmed', 'in-progress', 'completed', 'cancelled')
 * @returns Success/error response
 */
export async function updateSnowRequestStatus(
  requestId: string,
